...
public class EmployeeDBQuery {
    ...
    public static void main(String[] args) {
        ...
        Dataset<Row> employeeDataSet = sqlContext.read().jdbc(url, "employee", properties).cache();
        employeeDataSet.show();

        employeeDataSet.select(col("title")).distinct().show();
        employeeDataSet.filter("title in ('CFO', 'CEO')").show();

        Dataset<Row> employeeMacDataSet = sqlContext.read().jdbc(url, "employee", new String[]{"laptop = 'MAC'"}, properties).cache();
        employeeMacDataSet.show();

        Dataset<Row> employeeMacDataSet2 = sqlContext.read().jdbc(url, "(select * from employee where state = 'PA') AS tmp_table", properties).cache();
